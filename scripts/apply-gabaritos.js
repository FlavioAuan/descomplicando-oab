/**
 * Applies TIPO 1 (Branco) gabarito answers to phase1-raw.json.
 * Run: node scripts/apply-gabaritos.js
 */

const fs = require('fs')
const path = require('path')

const DATA_PATH = path.join(__dirname, 'data', 'phase1-raw.json')

// TIPO 1 answers extracted from PDFs. Index 0 = question 1.
// Uppercase letters — converted to lowercase when applied.
// 'ANULADA' = annulled question (stored as empty string).
const GABARITOS = {
...
for (const exam of data) {
  ...
  for (const q of exam.questions) {
    const answerRaw = answers[q.number - 1]
    if (!answerRaw) continue
    if (answerRaw === 'ANULADA') {
      q.correctAnswer = ''
    } else {
      q.correctAnswer = answerRaw.toLowerCase()
    }
    count++
  }

  console.log(`${exam.numeral}: applied ${count} answers`)
  totalUpdated += count
}

fs.writeFileSync(DATA_PATH, JSON.stringify(data, null, 2), 'utf8')
console.log(`\nDone. Total answers applied: ${totalUpdated}`)
